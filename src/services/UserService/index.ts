...
import { getProfileUrl, updateProfileUrl, updateAvatarProfile, changePasswordUrl, listUsersUrl, userUrl, banUserUrl, unbanUserUrl, createMentorUrl, createUserUrl } from './url'
...
export async function createUser(payload: CreateUserPayload): Promise<any> {
  const res: any = await api.post(createUserUrl, payload)
  clearUsersCache()
  return res?.data ?? res
}

export default { getProfile, updateProfile, uploadAvatarProfile, changePassword, listUsers, listUsersPaged, getUserById, banUser, unbanUser, createMentor, createUser, clearUsersCache }